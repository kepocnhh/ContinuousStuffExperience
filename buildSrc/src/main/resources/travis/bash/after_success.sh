...
git clone --depth=1 --branch="$TRAVIS_BRANCH" $REPO_URL.git $LOCAL_PATH || exit 4
...
echo "automatic merge not implemented..."
exit 0

GIT_URL="https://$git_hub_personal_access_token@github.com/$TRAVIS_REPO_SLUG"

echo $newline
echo "set upstream \"$TRAVIS_BRANCH\"..."
git -C $LOCAL_PATH branch -u -q $GIT_URL $TRAVIS_BRANCH || ILLEGAL_STATE=$?
if [ $ILLEGAL_STATE -ne 0 ]
then
  echo "Set upstream \"$TRAVIS_BRANCH\" failed!"
  exit $ILLEGAL_STATE
fi
